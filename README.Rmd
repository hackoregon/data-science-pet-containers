...
I've tried to provide a comprehensive command line experience. `Git`, `curl`, `wget`, `lynx` and `vim` are there, as is most of the command-line GIS stack (`gdal`, `proj`, `spatialite`, `rasterlite`, `geotiff`, `osm2pgsql` and `osm2pgrouting`), and of course `psql`. 
...
Docker will copy the backup files into `/home/postgres/Backups` on the `postgis` image, and place a script `restore-all.sh` in `/docker-entrypoint-initdb.d/`. The first time the image runs, `restore-all.sh` will restore all the `.backup` files it finds in `/home/postgres/Backups`. 
...
### Installing packages
As noted above, to keep the image size down, I've only provided a Jupyter notebook server. To install packages:
...
## RStudio
This service is based on the `rocker/rstudio` image from Docker Hub: <https://hub.docker.com/r/rocker/rstudio/>. I've added header files so that the R packages `RPostgres`, `odbc`, `sf` and `devtools` will install from source, but there are no R packages besides those that ship with `rocker/rstudio`.
