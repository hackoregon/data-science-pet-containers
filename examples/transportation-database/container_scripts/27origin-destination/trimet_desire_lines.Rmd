...
### Metro 2002
We start by tabulating the top 15 Oregon workplaces in the Portland metro area. We use `S000` (all jobs) as the jobs count [@LODES2018b].
```{r}
...
Tabulate the top 15 residences in the Portland metro area. We use `S000` (all jobs) as the workers count [@LODES2018b].
```{r}
top_metro_residences_2002 <- or_workers %>% filter(
  cbsaname == "Portland-Vancouver-Hillsboro, OR-WA",
  year == 2002
) %>% 
...
Now compute the flow - number of commuters into the workplaces by home and work census blocks. We remove entries where the home and work census blocks are the same.
```{r}
flow_metro_2002 <- or_od_JT00 %>% filter(
  ...
  arrange(desc(jobs)) %>% top_n(200, jobs)
```
...
qtm(top_metro_workplaces_2002, symbols.scale = 5, symbols.size = "jobs", symbols.col = "jobs") +
qtm(top_metro_residences_2002, symbols.scale = 5, symbols.size = "workers", symbols.col = "darkgreen")
...
top_metro_residences_2015 <- or_workers %>% filter(
  cbsaname == "Portland-Vancouver-Hillsboro, OR-WA",
  year == 2015
) %>% 
  mutate(workers = S000) %>% arrange(desc(workers)) %>% top_n(15, workers)
...
qtm(top_metro_workplaces_2015, symbols.scale = 5, symbols.size = "jobs", symbols.col = "jobs") +
qtm(top_metro_residences_2015, symbols.scale = 5, symbols.size = "workers", symbols.col = "darkgreen")
...
qtm(top_portland_workplaces_2015, symbols.scale = 5, symbols.size = "jobs", symbols.col = "jobs") +
qtm(top_metro_residences_2015, symbols.scale = 5, symbols.size = "workers", symbols.col = "darkgreen")
