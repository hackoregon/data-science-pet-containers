...
### Aggregate the workplaces
```{r}
workplaces <- or_od_main_JT00_2015 %>% 
  group_by(w_geocode) %>% summarize(Jobs = sum(S000)) %>% ungroup() %>% arrange(desc(Jobs)) %>% 
  left_join(or_xwalk, by = c("w_geocode" = "tabblk2010")) %>% 
  select(w_geocode, blklondd, blklatdd, Jobs) %>% 
  mutate(cumsum = cumsum(Jobs), cumfrac = cumsum / sum(Jobs))
workplaces <- SpatialPointsDataFrame(
  coords = select(workplaces, blklondd, blklatdd),
    data = select(workplaces, w_geocode, blklondd, blklatdd, Jobs, cumsum, cumfrac),
...
flow <- or_od_main_JT00_2015 %>%  
  ...
  arrange(desc(Jobs)) %>% top_n(100, Jobs)
```
...
### Make the desire lines and workplace points
```{r}
...
```{r}
block_polygons <- 
  read_sf("~/Raw/tl_2017_41_bg/tl_2017_41_bg.shp")  %>% filter(
    COUNTYFP == "005" |
      COUNTYFP == "051" |
      COUNTYFP == "067"
  )
...
tmap_mode("view")
qtm(desire_lines) + 
  qtm(workplaces, symbols.size = "Jobs", symbols.col = "black") + 
  qtm(block_polygons)
