...
```{r}
workplaces <- flow %>% select(w_geocode, S000) %>% unique() %>% 
  left_join(or_xwalk, by = c("w_geocode" = "tabblk2010")) %>% 
  select(w_geocode, blklondd, blklatdd, S000)
workplaces <- SpatialPointsDataFrame(
  coords = select(workplaces, blklondd, blklatdd),
    data = select(workplaces, w_geocode, blklondd, blklatdd, S000),
    proj4string = CRS("+init=epsg:4269")
)
...
tmap_mode("view")
qtm(desire_lines) + 
  qtm(workplaces, symbols.size = "S000", symbols.col = "firebrick") + 
  qtm(block_polygons)
