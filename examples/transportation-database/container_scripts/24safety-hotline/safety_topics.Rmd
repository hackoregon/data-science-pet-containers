...
Many of the terms we're interested in are bigrams - "bike lane", "stop sign", "speed bump", "street light" and so on. So we repeat the above anaylsis using bigrams for the terms [@silge2017text, chapter 4].

```{r}
raw_bigrams <- Safety_Hotline_Tickets %>% 
  select(Item_ID, Description) %>% 
...
bigrams_filtered <- bigrams_separated %>%
  filter(!word1 %in% pdx_stop_words$word) %>%
  filter(!word2 %in% pdx_stop_words$word)
...
bigram_tf_idf <- bigrams_united %>%
  count(Item_ID, bigram) %>%
  bind_tf_idf(bigram, Item_ID, n) %>%
  arrange(desc(tf_idf))
