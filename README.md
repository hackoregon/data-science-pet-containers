...
M. Edward (Ed) Borasky <znmeb@znmeb.net>, 2018-09-07
...
2.  Define the environment variables:
    ...
    -   Edit `.env`. The variables you need to define are
        ...
        -   `RSTUDIO_PASSWORD`: To log in to the `rstats` service, you
            need a user name and a password. The user name is the
            default, `rstudio`. Docker will set the password for the
            `rstudio` user in the `rstats` service to the value of
            `RSTUDIO_PASSWORD`.
...
Here’s `sample.env`:
    ...
    # rstats container
    RSTUDIO_PASSWORD=ir0ny.deficient
...
Browse to `localhost:8787`. The user name is `rstudio`. The password is
the value of `RSTUDIO_PASSWORD` you set in the `.env` file. ***Note that
if you’re using Firefox, you may have to adjust a setting to use the
terminal feature.***
